...
public class BalanceAdminCommand implements CommandExecutor {
    ...
    public BalanceAdminCommand(AzrovasEconomy plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        switch (subCommand) {
            case "set":
                ...
                if (args.length != 3) {
                    sender.sendMessage(ChatColor.RED + "Usage: /ecoadmin set [player] [amount]");
                    return true;
                }
                try {
            ...
            case "remove":
                ...
                if (args.length != 3) {
                    sender.sendMessage(ChatColor.RED + "Usage: /ecoadmin remove [player] [amount]");
                    return true;
                }
                try {
                    amount = Double.parseDouble(args[2]);
            ...
            case "add":
                ...
                if (args.length != 3) {
                    sender.sendMessage(ChatColor.RED + "Usage: /ecoadmin add [player] [amount]");
                    return true;
                }
                try {
                    amount = Double.parseDouble(args[2]);
            ...
            case "delete":
                ...
                if (args.length != 2) {
                     sender.sendMessage(ChatColor.RED + "Usage: /ecoadmin delete [player]");
                     return true;
                }
    ...
    private void sendUsage(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.YELLOW + "/ecoadmin set [player] [amount] - Set player's balance");
        sender.sendMessage(ChatColor.YELLOW + "/ecoadmin add [player] [amount] - Add money to player's balance");
        sender.sendMessage(ChatColor.YELLOW + "/ecoadmin remove [player] [amount] - Remove money from player's balance");
        sender.sendMessage(ChatColor.YELLOW + "/ecoadmin delete [player] - Delete player's economy account");
    }
